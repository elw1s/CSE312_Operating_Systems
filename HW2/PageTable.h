...
public:
    ...
    PageTableEntry(int referenced, int modified, int present, int page_frame_number, int time_of_the_last_use){
    ...
    void modify(int referenced, int modified, int present, int page_frame_number, int time_of_the_last_use){
        this->referenced = referenced;
        this->modified = modified;
        this->present = present;
        this->page_frame_number = page_frame_number;
    ...
    char * toString(){
        int bufferSize = snprintf(nullptr, 0, "%d-%d-%d-%d-%d", referenced, modified, present, time_of_the_last_use, page_frame_number);
        char* result = new char[bufferSize + 1];
        snprintf(result, bufferSize + 1, "%d-%d-%d-%d-%d", referenced, modified, present, time_of_the_last_use, page_frame_number);
        return result;
    }
};
